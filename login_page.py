...
def build_login_frame(parent, db_path, on_login_success=None, on_show_register=None):
    ...
    switchSignup = Button(bg_imageLogin, text="Sign Up", fg="#206DB4",
                          ...
                          command=(on_show_register if callable(on_show_register) else (lambda: None)))
    ...
    def login():
        try:
            email = Login_emailName_entry.get().strip()
            pwd = Login_passwordName_entry.get()
        except Exception:
            return
        ...
        ok = db_utils.verify_user(conn, email, pwd)
        conn.close()
        if ok:
    ...
    try:
        ...
        Login_button_1 = Button(bg_imageLogin, text="Login", bg="#206DB4", fg="#FFFFFF", command=login)
