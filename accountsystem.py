...
y = (AccountSystem.winfo_screenheight()//2) -(height//2)
AccountSystem.geometry('{}x{}+{}+{}'.format(width, height, x, y))

AccountSystem.title('Electric Bill Account System')
...
def signup():
    if firstName_entry.get() == "" or lastName_entry.get() == "" or emailName_entry.\
        ...
        try:
            ...
            connection.close()
            clear()
            messagebox.showinfo('Success', "New Account Created Succesfully :)")

        except Exception as es:
            messagebox.showerror("Error", "Something went wrong, plese try again :( ")
# =========================================================
#                       LOGIN pages starts here
# =========================================================
...
def login():
    ...
    cursor.execute(find_user, [(Login_passwordName_entry.get()), (Login_passwordName_entry.get())])

    result = cursor.fetchall()

    if result:
        messagebox.showinfo("Success", "Logged in Succesfully :)")
        login_win.destroy()  # close login
        electric_bill_gui()
    else:
        messagebox.showinfo("Failed", "Something went wrong TxT, Please try again")
...
#DATABASE CONNECTION FOR FORGOT PASSWORD
    ...
    def change_password():
        if email_entry3.get() == "" or new_password_entry.get() == "":
            messagebox.showerror("Error", "All Feilds are requred")
            exit.window()
            ...
            if row == None:
                ...
                db.commit()
                db.close()
                messagebox.showinfo('Confirmed', "Password Changed succesfully :)")
                exit.window()
